...
;;
;; The `gar-response-executor-*' symbol prefix is preserved verbatim for
...
(defgroup gar-response-executor nil
...
(defcustom gar-response-executor-auto-execute nil
...
mode is active (with confirmation if `gar-response-executor-confirm-before-execute')."
  :type 'boolean
  :group 'gar-response-executor)

(defcustom gar-response-executor-confirm-before-execute nil
...
(defcustom gar-response-executor-allowed-languages
  '("python" "sh" "bash" "elisp" "R" "ruby" "js")
  "Languages for which Babel auto-execution is allowed.
Checked in `gar-response-executor--execute-babel-block'."
  :type '(repeat string)
  :group 'gar-response-executor)

;;; --- Pattern-Based Auto Commands -----------------------------------

(defcustom gar-response-executor-auto-commands nil
...
(defcustom gar-response-executor-auto-execute-elisp nil
  "Reserved flag for future elisp auto-eval control.
Currently AUTORUN execution is coupled to `gar-response-executor-mode' and
`gar-response-executor-confirm-before-execute'."
  :type 'boolean
  :group 'gar-response-executor)

(defcustom gar-response-executor-elisp-tag "AUTORUN"
...
(defcustom gar-response-executor-allowed-functions nil
...
(defun gar-response-executor--find-babel-blocks ()
...
(defun gar-response-executor--lang-extension (lang)
...
(defun gar-response-executor--lang-command (lang file)
...
log in *Assistant Execution Results* and delete the temp file."
  (let* ((ext        (or (gar-response-executor--lang-extension lang) ""))
    ...
    (with-current-buffer result-buf
      ...
      (insert body)
      (insert "\n--- Output ---\n")
      (let ((cmd (gar-response-executor--lang-command lang tmp-file)))
        (if cmd
...
(defun gar-response-executor--file-output-block-p (block)
...
(defun gar-response-executor--execute-org-babel-file-block (block)
...
(defun gar-response-executor--execute-babel-block (block)
  "Execute a Babel block.
Checks `gar-response-executor-allowed-languages' and optionally
`gar-response-executor-confirm-before-execute'."
  (let ((lang (plist-get block :lang))
        (body (plist-get block :body)))
    (when (member lang gar-response-executor-allowed-languages)
      (cond
       ((gar-response-executor--file-output-block-p block)
        (if gar-response-executor-confirm-before-execute
            (when (yes-or-no-p
                   (format "Execute file-output block (%s)?\n%s\n" lang body))
              (gar-response-executor--execute-org-babel-file-block block))
          (gar-response-executor--execute-org-babel-file-block block)))
       (gar-response-executor-confirm-before-execute
        (when (yes-or-no-p
               (format "Execute block (%s):\n%s\n?" lang body))
          (gar-response-executor--do-execute lang body)))
       (t
        (gar-response-executor--do-execute lang body))))))

(defun gar-response-executor--find-commands ()
...
(defun gar-response-executor-run-command (command)
...
(defun gar-response-executor--find-autorun-blocks ()
  "Find elisp blocks whose header contains the AUTORUN tag.
Search depends on `gar-response-executor-elisp-tag'."
  (let (blocks)
    (save-excursion
      (goto-char (point-min))
      (while (re-search-forward
              (format "^#\\+begin_src elisp.*%s.*\n\\(\\(?:.*\n\\)*?\\)#\\+end_src"
                      gar-response-executor-elisp-tag)
              nil t)
        (push (match-string-no-properties 1) blocks)))
    (nreverse blocks)))

(defun gar-response-executor--whitelist-allows-p (code)
  "Check whether CODE (string) only calls functions from
`gar-response-executor-allowed-functions'.
If no whitelist is set → t (all allowed)."
  (if (null gar-response-executor-allowed-functions)
      t
    (let ((sexp    (read code))
          (allowed gar-response-executor-allowed-functions)
          (ok      t))
      (cl-labels ((walk (form)
                    (cond
...
(defun gar-response-executor--safe-eval (code)
...
Logs go to the buffer *Assistant Actions*."
  (let ((log-buf (get-buffer-create "*Assistant Actions*")))
    (cond
     ((not (gar-response-executor--whitelist-allows-p code))
...
(defun gptel-agent-runtime-repair-inline-plot-response (beg end)
...
;; Backwards-compat alias for the legacy `my/' name.
(defalias 'my/gptel-repair-inline-plot-response
  #'gptel-agent-runtime-repair-inline-plot-response)
  ...
  (when (and (stringp name) (fboundp 'gptel-tool-name))
    (cl-find-if (lambda (tool)
                ...
                (or (and (boundp 'gptel-tools) gptel-tools)
                    (and (fboundp 'gptel-agent-runtime-tools-all)
                         (gptel-agent-runtime-tools-all))))))
...
(defun gptel-agent-runtime-capability-summary ()
  ...
  (let* ((tools (sort (mapcar #'gptel-tool-name (or (gptel-agent-runtime-tools-all) nil))
                      #'string<))
...
(defun gar-response-executor-response-hook (beg end)
  "Hook for Babel blocks, exec-tags and auto-pattern matching.
Active only when `gar-response-executor-auto-execute' = t.
Registered in `gptel-post-response-functions'."
  (save-restriction
    ...
    (when gar-response-executor-auto-execute
      ;; 1. Repair obvious tutorial-style plot answers from weaker local models.
      (gptel-agent-runtime-repair-inline-plot-response (point-min) (point-max))
      ...
      ;; 3. Babel blocks
      (dolist (block (gar-response-executor--find-babel-blocks))
        (gar-response-executor--execute-babel-block block))
      ;; 4. Explicit exec-tags
      (dolist (cmd (gar-response-executor--find-commands))
        (when (yes-or-no-p (format "Execute command: %s ?" cmd))
          (gar-response-executor-run-command cmd)))
      ;; 5. User-defined patterns from gar-response-executor-auto-commands
      (dolist (pattern-cmd gar-response-executor-auto-commands)
        (save-excursion
          (goto-char (point-min))
          (when (re-search-forward (car pattern-cmd) nil t)
            (gar-response-executor-run-command (cdr pattern-cmd))))))))

(defun gar-response-executor-autorun-hook (beg end)
  "Hook for AUTORUN elisp blocks.
Always active while `gar-response-executor-mode' is on.
Respects `gar-response-executor-confirm-before-execute'."
  (save-restriction
    (narrow-to-region beg end)
    (dolist (code (gar-response-executor--find-autorun-blocks))
      (if gar-response-executor-confirm-before-execute
          (when (yes-or-no-p
                 (format "The assistant wants to execute:\n%s\nAllow?" code))
            (gar-response-executor--safe-eval code))
        (gar-response-executor--safe-eval code)))))

(defun gar-response-executor-execute-all-blocks ()
  "Manual variant: execute all Babel blocks in the current buffer."
  (interactive)
  (let ((blocks (gar-response-executor--find-babel-blocks)))
    (if blocks
        (dolist (block blocks)
          (gar-response-executor--execute-babel-block block))
      (message "No code blocks found."))))

(defun gar-response-executor-execute-block-at-point ()
...
(defvar gar-response-executor-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c C-x e") #'gar-response-executor-execute-all-blocks)
    (define-key map (kbd "C-c C-x b") #'gar-response-executor-execute-block-at-point)
    (define-key map (kbd "C-c C-x c") #'gar-response-executor-run-command)
    map)
  "Keymap for `gar-response-executor-mode'.")

(define-minor-mode gar-response-executor-mode
  "Global mode for automatic execution of assistant responses.
Activates the response and AUTORUN hooks on
`gptel-post-response-functions'."
  :lighter " CE"
  :keymap gar-response-executor-mode-map
  :global t
  (if gar-response-executor-mode
      (progn
        (add-hook 'gptel-post-response-functions
                  #'gar-response-executor-response-hook)
        (add-hook 'gptel-post-response-functions
                  #'gar-response-executor-autorun-hook)
...
;; Defgroup shim: keep `claude-executor' as a child of the new group so
;; M-x customize-group claude-executor still renders the same options.
(defgroup claude-executor nil
  "Compatibility shim for `gar-response-executor'.
This group exists only so user configs that referenced
`:group 'claude-executor' continue to work; the canonical group is
`gar-response-executor'."
  :group 'gar-response-executor)

;; Defcustoms (variables) -- old name -> new name.
(defvaralias 'claude-executor-auto-execute
  'gar-response-executor-auto-execute)
(defvaralias 'claude-executor-confirm-before-execute
  'gar-response-executor-confirm-before-execute)
(defvaralias 'claude-executor-allowed-languages
  'gar-response-executor-allowed-languages)
(defvaralias 'claude-executor-auto-commands
  'gar-response-executor-auto-commands)
(defvaralias 'claude-executor-auto-execute-elisp
  'gar-response-executor-auto-execute-elisp)
(defvaralias 'claude-executor-elisp-tag
  'gar-response-executor-elisp-tag)
(defvaralias 'claude-executor-allowed-functions
  'gar-response-executor-allowed-functions)

;; Private helpers -- old name -> new name.
(defalias 'claude-executor--find-babel-blocks
  'gar-response-executor--find-babel-blocks)
(defalias 'claude-executor--lang-extension
  'gar-response-executor--lang-extension)
(defalias 'claude-executor--lang-command
  'gar-response-executor--lang-command)
(defalias 'claude-executor--do-execute
  'gar-response-executor--do-execute)
(defalias 'claude-executor--file-output-block-p
  'gar-response-executor--file-output-block-p)
(defalias 'claude-executor--execute-org-babel-file-block
  'gar-response-executor--execute-org-babel-file-block)
(defalias 'claude-executor--execute-babel-block
  'gar-response-executor--execute-babel-block)
(defalias 'claude-executor--find-commands
  'gar-response-executor--find-commands)
(defalias 'claude-executor--find-autorun-blocks
  'gar-response-executor--find-autorun-blocks)
(defalias 'claude-executor--whitelist-allows-p
  'gar-response-executor--whitelist-allows-p)
(defalias 'claude-executor--safe-eval
  'gar-response-executor--safe-eval)

;; Public commands -- old name -> new name.
(defalias 'claude-executor-run-command
  'gar-response-executor-run-command)
(defalias 'claude-executor-response-hook
  'gar-response-executor-response-hook)
(defalias 'claude-executor-autorun-hook
  'gar-response-executor-autorun-hook)
(defalias 'claude-executor-execute-all-blocks
  'gar-response-executor-execute-all-blocks)
(defalias 'claude-executor-execute-block-at-point
  'gar-response-executor-execute-block-at-point)

;; Minor mode: alias both the function slot (defalias) and the value
;; slot (defvaralias) -- define-minor-mode creates a same-named variable
;; for the mode state in addition to the toggle function. Same for the
;; mode-map variable.
(defalias 'claude-executor-mode 'gar-response-executor-mode)
(defvaralias 'claude-executor-mode 'gar-response-executor-mode)
(defvaralias 'claude-executor-mode-map
  'gar-response-executor-mode-map)
