...
(defun gptel-agent-runtime--heading-matches-in-file (file heading)
  "Return list of buffer positions in FILE that match HEADING as a TODO line.
A match is `^*+ [STATE ]? HEADING <space|eol|colon>' so `Hello'
no longer accidentally matches `Hello2 Task' (regex without an
end-anchor) and so the modification tools can detect ambiguity
before acting.  Returns a list of (POSITION . LINE-TEXT) cons cells
in document order."
...
(defun gptel-agent-runtime--resolve-single-heading (file heading)
  "Resolve HEADING in FILE to a single (POSITION . LINE-TEXT) cons.
Returns either:
- nil with a `:status not-found' indicator (use `--heading-error-string')
- a single cons when exactly one match exists
- nil with `:status ambiguous' when multiple matches exist

This function is the gate every heading-modifying tool should run
through so silent multi-match edits stop happening."
  (let* ((p (expand-file-name file))
         (matches (gptel-agent-runtime--heading-matches-in-file p heading)))
    (cond
     ((null matches) (cons :not-found nil))
     ((= 1 (length matches)) (cons :found (car matches)))
     (t (cons :ambiguous matches)))))

(defun gptel-agent-runtime--heading-error-string (resolution file heading)
  "Format an error string for a non-`:found' RESOLUTION from
`--resolve-single-heading'.  Includes the file path and (for ambiguous
resolutions) the full list of matching lines so the model can rename
the heading argument to disambiguate."
  (pcase (car resolution)
    (:not-found
     (format "Heading not found in %s: %s" file heading))
    (:ambiguous
     (let ((matches (cdr resolution)))
       (format "Ambiguous: %d headings match '%s' in %s. Refine the heading text to disambiguate. Candidates:\n%s"
               (length matches) heading file
               (mapconcat (lambda (m) (format "  %s" (cdr m)))
                          matches "\n"))))
    (_ (format "Unknown heading resolution status: %S" (car resolution)))))
...
(with-eval-after-load 'gptel
  ...
  ;; Tightened (PR 13): regex now requires a word-boundary after the
  ;; heading text so `Hello' no longer matches `Hello2 Task', and
  ;; ambiguous matches return a list of candidates rather than silently
  ;; mutating every match.
  (gptel-make-tool
   ...
   :function (lambda (file heading state)
               (let* ((p (expand-file-name file))
                      (resolution (gptel-agent-runtime--resolve-single-heading
                                   p heading)))
                 (if (not (eq :found (car resolution)))
                     (gptel-agent-runtime--heading-error-string
                      resolution p heading)
                   (with-current-buffer (find-file-noselect p)
                     (save-excursion
                       (goto-char (car (cdr resolution)))
                       (org-todo state))
                     (save-buffer))
                   (format "State set to %s for: %s" state heading)))))

  ;; set_deadline -- set or update deadline on a heading.
  ;; Tightened (PR 13): same as above.
  (gptel-make-tool
   :name "set_deadline"
