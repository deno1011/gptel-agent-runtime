...
- =gptel-agent-runtime-run-injection-canaries=: Run the prompt-injection canary suite against the untrusted wrappers.
...
This module is a structural prompt-injection canary suite.

- `gptel-agent-runtime-run-injection-canaries' wraps known hostile payloads through `gptel-agent-runtime-untrusted-context'.
- Each result checks for required boundary and do-not-follow markers.
- Results are stored in `gptel-agent-runtime--last-canary-results' for mission control.
