#+TITLE: gar-quarantine: per-source quarantine + promotion + pre-flight conflict check
#+PROPERTY: header-args:emacs-lisp :tangle gar-quarantine.el :lexical t

Module of [[file:gptel-agent-runtime.org][gptel-agent-runtime]]. Implements per-source quarantine for
untrusted evidence (web / tool-result / file / experiment) and the
deterministic pre-flight that denies plan steps whose tool arguments
contain substrings from un-promoted quarantined evidence.
...
- [[file:gar-substrate.org][gar-substrate]] (provides =emit-event=, =--evidence-trace=, the evidence struct)
- [[file:gar-policy.org][gar-policy]] (calls =--quarantine-rule-text= from =untrusted-context=
  and =--quarantine-conflict-p= from =policy-evaluate-step=)
...
- =gptel-agent-runtime-evidence-quarantined-p=: Return non-nil when EVIDENCE is currently in quarantine.
- =gptel-agent-runtime-quarantined-evidence=: Return the list of evidence records currently in quarantine, newest first.
- =gptel-agent-runtime-promote-evidence=: Promote EVIDENCE-ID out of quarantine so its text may route tool calls.
- =gptel-agent-runtime--quarantine-rule-text=: Return the quarantine rule appended to untrusted wrappers when active.
- =gptel-agent-runtime--quarantine-conflict-p=: Return a deny-reason string when STEP arguments overlap quarantined text.
...
This module tracks untrusted evidence sources and blocks suspicious reuse until the user promotes the evidence.

- Untrusted evidence enters the trace through `gptel-agent-runtime-make-evidence' in `gar-substrate'.
- Context wrappers ask `gptel-agent-runtime--quarantine-rule-text' and `gptel-agent-runtime-evidence-quarantined-p' whether to append extra quarantine instructions.
- Policy evaluation calls `gptel-agent-runtime--quarantine-conflict-p' before tool execution.
- User trust flows through `gptel-agent-runtime-promote-evidence', which clears the quarantine and emits a policy-changed event.
