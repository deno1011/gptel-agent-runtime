...
This module A/B tests refined playbooks against originals.

- Experiments are started with `gptel-agent-runtime-experiment-start'.
- Session assignment flows through experiment lookup and deterministic arm picking.
- Outcome recording bumps original/candidate arm statistics.
- Decision helpers apply either a margin rule or Beta-Bernoulli Bayesian comparison.
- Public promote/rollback commands apply the decision, persist state, and emit experiment events.
