...
This module validates tool arguments against the package's small JSON-Schema-inspired subset.

- Policy and tests call `gptel-agent-runtime-validate-args' or `gptel-agent-runtime-args-valid-p'.
- Validation descends through internal `--validate-value', `--validate-object', `--validate-array', and `--validate-string' helpers.
- Object validation checks required keys, unknown keys, and nested property schemas.
- Primitive validation checks type, enum, min/max, length, and regexp pattern constraints.
