#+TITLE: gar-safety: mission-control unified dashboard
#+PROPERTY: header-args:emacs-lisp :tangle gar-safety.el :lexical t

Module of [[file:gptel-agent-runtime.org][gptel-agent-runtime]]. After the gar-safety sub-split landed
across 2026-05-27/28, this file is reduced to a single concern:
...
- /Mission-control unified dashboard/: =mission-control= buffer +
  ...
  =--mission-control-canary-summary=) that surfaces tick / event-pump /
  policy preset / quarantine size / canary status / skeptic verdicts /
  novelty + synthesis state / per-agent capability allowlists in one
  read-only buffer auto-refreshing on every =tick=.

The original responsibilities of gar-safety were factored out into:

- /gar-canaries/ (PR 1 of 5): prompt-injection canary suite.
- /gar-quarantine/ (PR 2 of 5): per-source quarantine + promotion +
  deterministic pre-flight conflict check.
- /gar-skeptic/ (PR 3 of 5): Advocatus Diaboli skeptic (rule-based +
  model-based).
- /gar-policy/ (PR 4 of 5): policy broker, capability gate, 5 named
  policy presets, =policy-evaluate-step=, =safety-check-step=,
  =confirmation-required-p=, untrusted/trusted context wrappers.

This module loads AFTER all of those so the dashboard can read each
submodule's defcustoms / defvars directly. PR 5 of the sub-split will
extract mission-control to =gar-mission-control= and delete this file
entirely.
