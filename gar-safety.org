#+TITLE: gar-safety: policy broker, capability gate, skeptic, quarantine, canaries, mission-control
#+PROPERTY: header-args:emacs-lisp :tangle gar-safety.el :lexical t

Module of [[file:gptel-agent-runtime.org][gptel-agent-runtime]]. The largest module of the split (~1100
lines of code). Concerns:

- /Policy broker/: protected paths, allowed-write-roots, risk levels,
  per-tool policy alist, the policy-decision flow
  (=policy-evaluate-step=, =safety-check-step=) and the path/agent/
  pattern checkers (=--policy-for-tool=, =--policy-default-allows-p=,
  =--policy-agent-allowed-p=, =--policy-path-allowed-p=,
  =--policy-command-blocked-p=, =--policy-confirmation-required-p=).
- /Policy presets/: the 5 named presets (=open=, =balanced=, =strict=,
  =research-only=, =coding-only=) and =apply-policy-preset= plus the
  =set-policy-preset= alias.
- /Zero-trust capability layer/: =capability-vocabulary=,
  =tool-capabilities= manifest, =--default-caps-from-risk=,
  =caps-for-tool=, =resolve-agent-caps=, =--caps-subset-p=,
  =--capability-check= stacked BEFORE the per-tool policy alist.
- /Untrusted-context wrapping/: =--truncate-context=,
  =untrusted-context= (now provenance-aware with evidence struct
  support), =trusted-context= -- the prompt-shaped wrappers every
  module uses when reinjecting tool/file/web/memory output into a
  prompt.
- /Per-source quarantine/: defcustoms,
  =evidence-quarantined-p=, =quarantined-evidence=, =promote-evidence=,
  =--quarantine-rule-text=, =--quarantine-conflict-p= (the
  deterministic pre-flight that denies tool calls whose args overlap
  with un-promoted quarantined evidence text).
- /Advocatus Diaboli skeptic/: defcustoms,
  =--skeptic-applies-p=, =--skeptic-rule-based-verdict=,
  =skeptic-evaluate=, =--apply-skeptic-to-decision= (mutates the
  policy decision to force confirmation on =high= verdicts).
- /Prompt-injection canaries/: =injection-canaries= defcustom,
  =run-injection-canaries= asserting wrapper integrity.
- /Mission-control unified dashboard/: =mission-control= buffer,
  section helpers (=--mission-control-section=,
  =--mission-control-recent-events=, =--mission-control-recent-evidence=,
  =--mission-control-canary-summary=), plus =show-guardrails= and the
  guardrails summary.

See also:
- [[file:gptel-agent-runtime.org][gptel-agent-runtime]] (master entrypoint that requires this module)
- [[file:gar-substrate.org][gar-substrate]] (provides =emit-event=, =make-evidence=,
  =evidence-trace=, and the event taxonomy this module dispatches on)
- [[file:gar-tools.org][gar-tools]] (raw-tool allow-list defcustoms in the master are read
  by gar-tools' shim; the capability manifest in this module declares
  per-tool caps that the gate enforces against agent allowed-caps)
- [[file:gar-memory.org][gar-memory]] (mission-control surfaces evidence + playbook stats)
...
* Per-source quarantine, prompt-injection canaries, mission-control
...
* Policy broker, capability gate, skeptic, policy presets
...
* Untrusted/trusted context wrappers (formerly in the Tool Registry source block)
