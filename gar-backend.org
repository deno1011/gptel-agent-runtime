...
- model-id normalization helpers: =my/gptel-model-id= (returns the
  symbol id from gptel metadata in any shape -- symbol, cons, vector)
  and =my/gptel-backend-model-symbols= (lists a backend's model ids
  uniformly as symbols)
- =my/gptel-register-model= -- adds (or replaces) one entry in the
...
- [[file:gar-directives.org][gar-directives]] (=my/gptel-sync-directive-for-current-runtime= is
...
a vector. =my/gptel-model-id= picks the symbol id regardless of which
shape gptel hands back, so downstream code can compare model ids with
=eq=. =my/gptel-backend-model-symbols= lists a backend's known models
...
3. Populating =my/gptel-backends= with the resulting
   ...
   or per-entry via =my/gptel-register-model=).
...
See =~/emacs/config/gptel-setup.org= (live private setup) and
