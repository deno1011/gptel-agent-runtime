...
- forward =defvar= for =my/gptel-backends= (alist of registered backends
  populated by the setup file) and =my/gptel-ollama-backend= (the live
...
- =gptel-agent-runtime-register-model= -- adds (or replaces) one entry in the
  =my/gptel-backends= alist; called by the user's backend setup and by
  the model-router
...
The setup file populates =my/gptel-backends= directly with =setq=, but
...
2. Calling =gptel-make-anthropic= / =gptel-make-openai= /
   ...
   and model lists.
3. Populating =my/gptel-backends= with the resulting
