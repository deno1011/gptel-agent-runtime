...
This module mediates between runtime model selection and host-defined gptel backends.

- Ollama helpers build URLs, probe server availability, optionally start Ollama, and detect the currently loaded model.
- Model normalization converts gptel metadata shapes into stable symbols.
- `gptel-agent-runtime-use-default-local-model' chooses the active Ollama model or configured fallback, then syncs directives/tools.
- `gptel-agent-runtime-register-model' updates the backend/model registry consumed by selection UI and model routing.
