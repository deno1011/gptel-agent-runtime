#+TITLE: gar-tools: tool registry + raw-tool helpers + native gptel tools + tool-invention pipeline
#+PROPERTY: header-args:emacs-lisp :tangle gar-tools.el :lexical t

Module of [[file:gptel-agent-runtime.org][gptel-agent-runtime]]. Three concerns share this file because they
share the same data model (the =gptel-agent-runtime-tool= struct and the
=gptel-agent-runtime-tool-registry= alist):

1. /Tool registry abstraction/: =cl-defstruct gptel-agent-runtime-tool= and
   the =register-tool= / =find-tool= / =tools-by-category= helpers, plus
   the =gptel-agent-runtime-action-result= struct + =result-ok= /
   =result-error= / =result-pending-reflection-p= used by every tool
   invocation site.
2. /Native gptel tool registrations/: the
   =(with-eval-after-load 'gptel (gptel-make-tool ...))= calls for every
   tool the runtime exposes to a model -- =describe_capabilities=,
   Org/file/buffer/code/export/web tools.
3. /Tool-invention pipeline/: =propose-tool=, the
   =--safe-form-violations= static analyzer, the subprocess validator
   (=emacs -Q --batch= byte-compile with =with-timeout=), and the
   =approve-= / =reject-= / =review-proposed-tools= command surface.

The raw-tool JSON compatibility shim that catches local-model JSON-as-text
already lives in [[file:gar-executor.org][gar-executor]]; the allow-list defcustoms it consults
(=gptel-agent-runtime-raw-tool-call-names=,
=-raw-tool-confirmation-names= ...) remain in the master for now.

See also:
- [[file:gptel-agent-runtime.org][gptel-agent-runtime]] (master entrypoint that requires this module)
- [[file:gar-executor.org][gar-executor]] (consumes the raw-tool allow-lists; its raw-tool shim
  reads them through plain symbol lookup at execute time)
...
- =gptel-agent-runtime-register-tool=: Register tool metadata.
- =gptel-agent-runtime-find-tool=: Return registered tool metadata for NAME, or nil.
- =gptel-agent-runtime-tools-by-category=: Return all registered tools in CATEGORY.
- =gptel-agent-runtime-result-ok=: Create a successful action result.
- =gptel-agent-runtime-result-error=: Create a failed action result.
- =gptel-agent-runtime-result-pending-reflection-p=: Return non-nil when RESULT should trigger a reflection step.
- =gptel-agent-runtime--proposed-tools-directory=: Return (and create) the proposed-tools directory.
- =gptel-agent-runtime--rejected-tools-directory=: Return (and create) the rejected-tools directory.
- =gptel-agent-runtime--symbol-allowed-prefix-p=: Return non-nil when SYM's name starts with an allowed prefix.
- =gptel-agent-runtime--safe-form-violations=: Walk FORM and return a list of (SYMBOL .
- =gptel-agent-runtime--read-all-forms=: Read all top-level Lisp forms from FILE and return them as a list.
- =gptel-agent-runtime-propose-tool=: Save a tool proposal under proposed-tools/ and emit a submission event.
- =gptel-agent-runtime--read-proposal=: Return the proposal plist stored in FILE, or nil when unreadable.
- =gptel-agent-runtime--locate-emacs-binary=: Return an absolute path to the running Emacs binary, or nil.
- =gptel-agent-runtime-validate-proposed-tool=: Validate the proposal at FILE through static + subprocess checks.
- =gptel-agent-runtime-list-proposed-tools=: Return the list of proposed-tool files awaiting review.
- =gptel-agent-runtime-approve-proposed-tool=: Approve and register the proposed tool from FILE.
- =gptel-agent-runtime-reject-proposed-tool=: Move FILE to the rejected/ subdirectory.
- =gptel-agent-runtime-review-proposed-tools=: Open a buffer summarizing proposed tools and their validation status.
- =gptel-agent-runtime--path-protected-p=: Return non-nil when PATH is protected from agent writes.
- =gptel-agent-runtime--heading-matches-in-file=: Return list of buffer positions in FILE that match HEADING as a TODO line.
- =gptel-agent-runtime--resolve-single-heading=: Resolve HEADING in FILE to a single (POSITION .
- =gptel-agent-runtime--heading-error-string=: Format an error string for a non-`:found' RESOLUTION from `--resolve-single-heading'.
...
This module owns the runtime tool registry, native gptel tools, argument schemas, and tool-invention review.

- Tool registration flows through `gptel-agent-runtime-register-tool', then lookup/category helpers feed policy and worker dispatch.
- Result constructors normalize successful, failed, and pending-reflection tool outcomes.
- Native tools provide file, buffer, Org, web, memory, and execution operations used by plan steps.
- High-risk tools have argument schemas consumed by `gar-validator' through `gar-policy'.
- Tool invention writes proposed Elisp, statically scans denied forms/prefixes, byte-compiles in a subprocess, and requires explicit approval before registration.
...
Package-shaped registry for tool metadata. Tools register a
=gptel-agent-runtime-tool= struct alongside their =gptel-make-tool=
counterpart so the runtime can introspect capability/category/risk
without poking at gptel internals. The =action-result= struct
normalizes every tool's return value so the loop can reason about
status uniformly.
...
End-to-end pipeline for runtime-proposed new tools: =propose-tool= writes
a versioned proposal file; =--safe-form-violations= statically checks
the proposed body against a denied-form list (and an allowed-prefix
allowlist for gar-* / gptel-* callers); =validate-proposed-tool= spawns
an =emacs -Q --batch= subprocess with =with-timeout= to byte-compile the
proposal in isolation; =M-x gptel-agent-runtime-approve-proposed-tool=
registers the approved tool through =gptel-agent-runtime-register-tool=
and emits =tool-proposal-approved=. =M-x review-proposed-tools= shows
pending proposals with their static + subprocess status.
...
* Native gptel tool registrations

Every tool the runtime exposes to a model is registered here via
=gptel-make-tool=. Tools register lazily through
=(with-eval-after-load 'gptel ...)= so the registrations also fire on
delayed gptel installs. The full set covers Org introspection, code
execution, file system access, buffer inspection, export, and web
fetch.
...
* Arg schemas for the high-risk native tools

Schemas declared via the new =:arg-schema= keyword to
=gptel-agent-runtime-register-tool=. They mirror the =gptel-make-tool=
=:args= declarations above with additional structural constraints
(non-empty strings, allowed languages, date pattern, tag charset,
=:additional-properties nil= to catch stray keys the model might
hallucinate). The =gptel-agent-runtime= registry sits alongside the
=gptel= registry; the policy broker (=gar-policy=) looks up the schema
during =policy-evaluate-step= and converts any violations into a
deny-reason. Tools without a registered schema are unchanged.

Only the mutation-capable + code-exec tools are backfilled here; read-
only tools (=read_file=, =read_org_file=, =web_search=, etc.) carry
less risk from malformed args and are left for later passes when a
