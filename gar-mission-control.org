...
- =gptel-agent-runtime--mission-control-section=: Insert a TITLE section with BODY (a string) into the current buffer.
- =gptel-agent-runtime--mission-control-recent-events=: Return a string with the LIMIT most recent dispatched events for the dashboard.
- =gptel-agent-runtime--mission-control-recent-evidence=: Return a string with the LIMIT most recent evidence records.
- =gptel-agent-runtime--mission-control-canary-summary=: Return a string describing the most recent canary run.
- =gptel-agent-runtime-mission-control=: Open the unified mission-control dashboard buffer.
- =gptel-agent-runtime--tool-policy-known-tools=: Return the union of tool names from the capability manifest and policy alists.
- =gptel-agent-runtime--tool-policy-source=: Return symbol describing which layer supplies TOOL's effective policy.
- =gptel-agent-runtime--tool-policy-row=: Return the tabulated-list row vector for TOOL.
- =gptel-agent-runtime--tool-policy-rows=: Return all rows for the editor, one per known tool.
- =gptel-agent-runtime--tool-policy-current-tool=: Return the tool name on the current line, or nil.
- =gptel-agent-runtime--tool-policy-current-entry=: Return the current USER policy plist for TOOL (copy), or nil.
- =gptel-agent-runtime--tool-policy-set-entry=: Replace TOOL's entry in `gptel-agent-runtime-tool-policy' with PLIST.
- =gptel-agent-runtime--tool-policy-cycle-key=: Cycle TOOL's :KEY through CYCLE in the user policy alist.
- =gptel-agent-runtime--tool-policy-strip-key=: Return PLIST with KEY removed; return nil if the resulting plist is empty.
- =gptel-agent-runtime-tool-policy-cycle-confirm=: Cycle the :confirm key for the tool on the current line.
- =gptel-agent-runtime-tool-policy-cycle-default=: Cycle the :default key for the tool on the current line.
- =gptel-agent-runtime-tool-policy-cycle-taint=: Cycle the :taint key for the tool on the current line.
- =gptel-agent-runtime-tool-policy-reset-line=: Remove the user override for the tool on the current line.
- =gptel-agent-runtime-tool-policy-save=: Persist the current `gptel-agent-runtime-tool-policy' via Customize.
- =gptel-agent-runtime-tool-policy-editor=: Open the tabulated-list editor for `gptel-agent-runtime-tool-policy'.
...
This module is the dashboard and tool-policy editor.

- `gptel-agent-runtime-mission-control' builds the dashboard buffer from live state across modules.
- Section helpers render recent events, evidence, canaries, policy posture, verifier/skeptic state, failure analytics, experiments, and skill candidates.
- Tool-policy editor entry flows through `gptel-agent-runtime-tool-policy-editor'.
- The editor renders rows with `gptel-agent-runtime--tool-policy-row' / `--tool-policy-rows' and mutates entries through cycle/reset/save commands.
- Policy edits update `gptel-agent-runtime-tool-policy' and can be saved back to customization.
