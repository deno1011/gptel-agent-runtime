...
(defun gptel-agent-runtime--skill-promote-candidate-files ()
  "Return the list of candidate file paths in the auto-synth directory.
...
(defun gptel-agent-runtime--skill-promote-rows ()
  "Build `tabulated-list-entries' for the review buffer."
  (let ((files (gptel-agent-runtime--skill-promote-candidate-files)))
    (mapcar
     (lambda (file)
       (let* ((name (file-name-base file))
         ...
         (list file (vector name mtime size))))
     (or files '()))))
...
(defun gptel-agent-runtime-skill-promote-approve ()
  "Move the candidate on the current line into the active skills
directory and (when `skills-md' is loaded) register it as a playbook."
  (interactive)
  (let* ((src (gptel-agent-runtime--skill-promote-current-file))
         (skills-dir (or (and (boundp 'gptel-agent-runtime-skills-directory)
                              gptel-agent-runtime-skills-directory)
                         user-emacs-directory))
    ...
    (gptel-agent-runtime-emit-event
     ...
     :source "skill-promote"
     :payload (list :file dst
                    :id (and skill (plist-get skill :id)))
     :taint 'trusted)
    (when (derived-mode-p 'gptel-agent-runtime-skill-promote-review-mode)
      (tabulated-list-revert))
    (message "Approved: %s -> %s" (file-name-base src) dst)))

;;;###autoload
(defun gptel-agent-runtime-skill-promote-reject ()
  "Move the candidate on the current line into the
auto-synth/rejected/ subdirectory.  Recoverable -- the file
isn't deleted, just shelved."
  (interactive)
  (let* ((src (gptel-agent-runtime--skill-promote-current-file))
    ...
    (when (derived-mode-p 'gptel-agent-runtime-skill-promote-review-mode)
      (tabulated-list-revert))
    (message "Rejected: %s" (file-name-base src))))
