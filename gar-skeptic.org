...
- =gptel-agent-runtime--skeptic-applies-p=: Return non-nil when the skeptic should fire for RISK and REQUIRED-CAPS.
- =gptel-agent-runtime--skeptic-rule-based-verdict=: Return a rule-based skeptic verdict plist.
- =gptel-agent-runtime-skeptic-evaluate=: Return a skeptic verdict for STEP given the policy DECISION, or nil.
- =gptel-agent-runtime--apply-skeptic-to-decision=: Mutate DECISION metadata with VERDICT and escalate confirmation for `high'.
...
This module is the deterministic Advocatus Diaboli gate used by policy evaluation.

- `gptel-agent-runtime--skeptic-applies-p' decides whether a step is risky enough to review.
- `gptel-agent-runtime--skeptic-rule-based-verdict' catches obvious risky patterns without a model call.
- `gptel-agent-runtime-skeptic-evaluate' dispatches to rule-based or model-based mode.
- `gptel-agent-runtime--apply-skeptic-to-decision' attaches the verdict to an allowed policy decision and can escalate confirmation.
