...
`lexical' uses local keyword scoring. `embeddings' asks the local server for
embeddings and falls back to lexical scoring when unavailable."
  :type '(choice (const :tag "Lexical" lexical)
                 (const :tag "Ollama embeddings" embeddings))
  :group 'gptel-agent-runtime)

(defcustom gptel-agent-runtime-embedding-model "nomic-embed-text"
  "Local embedding model used when memory retrieval method is `embeddings'."
  :type 'string
  :group 'gptel-agent-runtime)

(defcustom gptel-agent-runtime-embedding-cache-enabled t
  "When non-nil, persist local embeddings in a local cache file."
  :type 'boolean
  :group 'gptel-agent-runtime)
...
(defun gptel-agent-runtime-memory-retrieve (query &optional limit)
  ...
  (let* ((limit (or limit 3))
         scored)
    (dolist (file (directory-files gptel-agent-runtime-memory-directory t "\\.el$"))
      (unless (member (file-name-nondirectory file)
                      '("playbooks.el" "skill-stats.el" "embedding-cache.el"))
        (with-temp-buffer
          (insert-file-contents file)
          (goto-char (point-min))
          (let* ((data (condition-case nil (read (current-buffer)) (error nil)))
                 (text (plist-get data :text))
                 (text-snippet (truncate-string-to-width text 1800 nil nil t))
