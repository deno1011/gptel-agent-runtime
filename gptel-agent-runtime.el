...
(defun gptel-agent-runtime--delphi-system ()
  ...
  "You are an isolated specialist in a Delphi-style peer process. Produce an independent concise draft. Do not mention other agents. Focus on your assigned role, assumptions, risks, and recommended next steps.")
...
(defun gptel-agent-runtime--aggregate-delphi-drafts (session drafts)
  ...
  (let* ((task (gptel-agent-runtime-session-current-task session))
    ...
    (gptel-request
     prompt
     :system "You are a Delphi aggregator. Synthesize anonymous specialist drafts into a concise final answer. Do not expose agent identities unless useful. Mention uncertainty and disagreements."
     :callback
     (lambda (response _info)
       (if response
