...
;; NOTE: Backend registration intentionally lives outside this package.
;; The consuming setup file is responsible for:
;;   1. Requiring provider modules:
;;        (require 'gptel-anthropic nil t)
;;        (require 'gptel-openai nil t)
;;        (require 'gptel-ollama nil t)
;;        (require 'gptel-gemini nil t)
;;   2. Calling gptel-make-anthropic / gptel-make-openai / gptel-make-ollama
;;      with the user's preferred endpoints, API keys, and model lists.
;;   3. Populating `my/gptel-backends' with the resulting (DISPLAY-NAME .
;;      (BACKEND . MODEL)) entries.
;;   4. Optionally setting `my/gptel-ollama-backend' when an Ollama backend
;;      is registered, so `gptel-agent-runtime-use-default-local-model' and
;;      mission-control can find it.
...
;; See:
;;   ~/emacs/config/gptel-setup.org      (live private setup)
;;   ~/emacs-mac-setup/gptel-setup.org   (bootstrap template)
