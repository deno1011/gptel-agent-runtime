...
(defun gptel-agent-runtime--raw-tool-call-confirmed-p (call risk)
  "Return non-nil when raw tool CALL with RISK may execute."
  (or (not (gptel-agent-runtime-confirmation-required-p risk))
    ...
    (cond
     ...
     ((gptel-agent-runtime-safety-check-step step)
      (format "Skipped raw tool call `%s': %s"
              name
              (gptel-agent-runtime-safety-check-step step)))
     ((not (gptel-agent-runtime--raw-tool-call-confirmed-p call risk))
      ...
      (if (>= gptel-agent-runtime--raw-tool-continuation-depth
        ...
        (let (observations)
          ...
          (let ((successful (cl-remove-if-not
            ...
            (gptel-agent-runtime--request-raw-tool-continuation successful)
            successful))))))
...
(defun gptel-agent-runtime--parallel-safe-step-p (step)
  ...
  (let* ((tool-name (or (gptel-agent-runtime-plan-step-suggested-tool step)
    ...
    (and (or read-safe mutation-safe)
         (not (gptel-agent-runtime-safety-check-step step)))))
