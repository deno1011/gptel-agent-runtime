...
(defun gptel-agent-runtime-untrusted-context (label text &optional source)
  "Wrap TEXT as untrusted evidence named LABEL from optional SOURCE."
  (let ((text (gptel-agent-runtime--truncate-context text)))
    (if (not gptel-agent-runtime-wrap-untrusted-context)
        text
      (format (concat "=== BEGIN UNTRUSTED %s%s ===\n"
              ...
              (if source (format " FROM %s" source) "")
              text
              (upcase (or label "CONTEXT"))))))

(defun gptel-agent-runtime-trusted-context (label text)
  "Wrap trusted runtime TEXT with LABEL for prompt readability."
  (format "=== BEGIN TRUSTED %s ===\n%s\n=== END TRUSTED %s ==="
          (upcase (or label "CONTEXT"))
          (format "%s" (or text ""))
          (upcase (or label "CONTEXT"))))
