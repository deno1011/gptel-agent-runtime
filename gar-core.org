...
- /Executor mode activation/: the =claude-executor-*= setq block
  and =(claude-executor-mode 1)= call at the end (these only kick
  in once =gar-executor= has actually defined the mode).
