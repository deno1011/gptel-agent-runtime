#+TITLE: gar-core: package metadata, defcustoms, defvars, structs, base helpers
#+PROPERTY: header-args:emacs-lisp :tangle gar-core.el :lexical t

Module of [[file:gptel-agent-runtime.org][gptel-agent-runtime]]. The lowest layer of the runtime, loaded
first by the master entrypoint so every other module finds the
=defcustom=, =defvar=, =cl-defstruct=, and base helpers it needs.

What this module owns:

- /Package metadata/: =defgroup gptel-agent-runtime=,
  =gptel-agent-runtime-package-name=, =-public-prefix=,
  =-private-prefix=, =-state-schema-version=.
- /All defcustoms/: user-facing options that other modules
  consult by symbol -- chat router, model router, event log, policy
  presets, embedding model, parallel workers, raw-tool allow lists,
  trace buffer names, swarm trace, status markers, plan review,
  Ollama host, idle pump interval, tool capabilities manifest,
  default tool policy, default Delphi agents, default role/process,
  organization routing, playbook learning, memory retrieval method,
  novelty/synthesis thresholds, hypothesis-test, tool-invention
  denied-form list and prefix allowlist, untrusted-context max
  chars, quarantine pre-flight, skeptic budget/mode, mission-control
  buffer name, model-router profiles, ...
- /All defvars (non-module)/: state used by helpers and lifecycle
  glue (skill stats, last-route, last-skeptic-verdicts, etc.).
- /All cl-defstructs/: =gptel-agent-runtime-task=, =session=,
  =worker=, =plan-step=, =plan=, plus the runtime-internal helpers
  that read/write them.
- /Base helpers/: =--symbol-name=, =--tokenize-text=,
  =--trigger-matches-p=, =--plist-values-for-keys=,
  =--normalize-args=, =--path-under-directory-p=,
  =protected-path-p=, risk-level helpers, =--find-native-tool= /
  =--call-native-tool= used by the loop.
- /Compatibility glue/: =gptel-agent-runtime-tools-all=, =gptel-agent-runtime-sync-tools=,
  =gptel-agent-runtime-select-model=, =gptel-agent-runtime-mode-line=, =gptel-agent-runtime-status=,
  =gptel-agent-runtime-sync-and-status=, the C-c bindings for status/select.
- /Executor mode activation/: the =gar-response-executor-*= setq block
  and the activation call at the master's tail (these only kick in
  once =gar-executor= has actually defined the mode). The
  =claude-executor-*= legacy names remain available via
  =defvaralias= / =defalias= shims registered by =gar-executor=.

This module loads first; every other =gar-*= module references its
symbols, frequently via plain function calls (late-bound at runtime)
or explicit =defvar= / =declare-function= forward declarations.

See also:
- [[file:gptel-agent-runtime.org][gptel-agent-runtime]] (master entrypoint that requires this module
  before any other =gar-*= module)
- every other =gar-*= module references defcustoms/defvars/structs
  defined here
...
- =gptel-agent-runtime-create-task=: Create a task object with TITLE, GOAL, and optional PARENT-ID.
- =gptel-agent-runtime-create-session=: Create a session around ROOT-TASK using ROLE or the default role.
- =gptel-agent-runtime-create-plan=: Create a plan for TASK with optional STEPS.
- =gptel-agent-runtime-create-plan-step=: Create one draft plan step.
- =gptel-agent-runtime-plan-complete-p=: Return non-nil when every step in PLAN is done.
- =gptel-agent-runtime-next-plan-step=: Return the first non-done step in PLAN.
- =gptel-agent-runtime-tools-all=: Return list of all registered gptel-tool structs.
- =gptel-agent-runtime-sync-tools=: Enable all registered gptel tools in the current buffer.
- =gptel-agent-runtime-select-model=: Select backend+model — sets globally AND in the active gptel buffer.
- =gptel-agent-runtime-mode-line=: Show the active gptel model in the mode line.
- =gptel-agent-runtime-status=: Display active gptel backend, model, directive, and tool count.
- =gptel-agent-runtime-sync-and-status=: Synchronize the directive for the active runtime and display status.
...
This module is the shared vocabulary and state foundation. It mostly runs at load time, then other modules consume the constructors, structs, and settings.

- Load time defines the defgroup, user options, compatibility aliases, structs, and base helper functions.
- Session construction flows through `gptel-agent-runtime-create-task' -> `gptel-agent-runtime-create-session' -> `gptel-agent-runtime-create-plan' -> `gptel-agent-runtime-create-plan-step'.
- Plan readers use `gptel-agent-runtime-plan-complete-p' and `gptel-agent-runtime-next-plan-step' to decide what is still pending.
- Tool setup flows through `gptel-agent-runtime-tools-all' and `gptel-agent-runtime-sync-tools'.
- User inspection flows through `gptel-agent-runtime-select-model', `gptel-agent-runtime-mode-line', `gptel-agent-runtime-status', and `gptel-agent-runtime-sync-and-status'.
...
* Package metadata, defcustoms, defvars, structs, helpers (consolidated from the master)
