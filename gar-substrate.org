...
- /Provenance/: =make-evidence= constructor + =--evidence-header-tag=
  ...
  single mutable history that =gar-quarantine=, =gar-policy=, and
  =gar-memory= consult.
...
This module is the event/evidence substrate. Runtime work emits events here; subscribers in later modules observe those events without tight coupling.

- `gptel-agent-runtime-emit-event' is the main entry point: it advances the tick, records/logs the event, and dispatches it.
- `gptel-agent-runtime--advance-tick' and `gptel-agent-runtime--emit-tick-event' keep tick events non-recursive.
- `gptel-agent-runtime-subscribe' and `gptel-agent-runtime-unsubscribe' maintain event subscribers; `gptel-agent-runtime--dispatch-event' calls each handler safely.
- Evidence creation flows through `gptel-agent-runtime-make-evidence', then `gptel-agent-runtime-trace-evidence' renders provenance.
- State persistence helpers `gptel-agent-runtime--state-header', `gptel-agent-runtime--read-versioned', and `gptel-agent-runtime-migrate-state' protect future migrations.
- UI helpers `gptel-agent-runtime-show-swarm' and `gptel-agent-runtime-show-event-pump' expose the event stream.
