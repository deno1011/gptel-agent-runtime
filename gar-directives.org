...
- the selection helpers that decide which directive applies to the
  current model: =gptel-agent-runtime-directive-for-choice=, =gptel-agent-runtime-local-runtime-p=,
...
- [[file:gar-context.org][gar-context]] (the =gptel-agent-runtime-web-search-ddg= / =gptel-agent-runtime-web-text= helpers referenced
...
- =gptel-agent-runtime-directive-for-choice=: Return the directive symbol that should be used for model CHOICE.
- =gptel-agent-runtime-local-runtime-p=: Return non-nil when the active gptel backend/model is local.
- =gptel-agent-runtime-directive-for-current-runtime=: Return the directive symbol for the active gptel backend/model.
- =gptel-agent-runtime-sync-directive-for-current-runtime=: Set `gptel--system-message' according to the active backend/model.
- =gptel-agent-runtime-current-directive-name=: Return the directive symbol that matches the current system message.
...
This module selects and synchronizes gptel system directives.

- Runtime checks identify local backends/models with `gptel-agent-runtime-local-runtime-p'.
- `gptel-agent-runtime-directive-for-current-runtime' chooses the local-assistant or default assistant directive.
- `gptel-agent-runtime-sync-directive-for-current-runtime' installs that directive into gptel state before sends/model switches.
- Directive strings encode tool-calling, raw-tool fallback, faithfulness, and local-model behavior expectations.
...
They are called from =gptel-agent-runtime-select-model=, the autonomous loop, the
