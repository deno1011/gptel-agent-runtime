...
;;  - the selection helpers (`gptel-agent-runtime-directive-for-choice',
;;    `gptel-agent-runtime-local-runtime-p', `gptel-agent-runtime-directive-for-current-runtime',
;;    `gptel-agent-runtime-sync-directive-for-current-runtime',
;;    `gptel-agent-runtime-current-directive-name').
...
(defvar gptel-agent-runtime-local-runtime-pattern
...
(defun gptel-agent-runtime-directive-for-choice (_choice)
  "Return the directive symbol that should be used for model CHOICE.

History: this used to branch on the runtime, returning the thin
`emacs-assistant' directive for non-local backends.  The thin directive
turned out to under-serve the 7B-tier remote models (Haiku, gpt-4o-mini)
that have re-emerged below the original API price floor -- they need
the same hand-holding (CRITICAL RULES, CRITICAL REASONING, TOOL OUTPUT
FAITHFULNESS, ORG FILES guidance) that the local-runtime directive
provides to local 7B/14B models.  We now route every backend through
the rich directive; large frontier models pay a small input-token tax
but behave more consistently as a result.

CHOICE is preserved as an argument for backwards compatibility; it is
no longer consulted."
  'emacs-local-assistant)

(defun gptel-agent-runtime-local-runtime-p ()
  "Return non-nil when the active gptel backend/model is local.
Still used by callers that need to know whether Ollama / LM Studio /
MLX is in play (for example to skip API rate-limit checks), even now
that directive selection no longer branches on this."
...
(defun gptel-agent-runtime-directive-for-current-runtime ()
  "Return the directive symbol for the active gptel backend/model.
Always returns `emacs-local-assistant'.  See `directive-for-choice'
for the rationale -- the thin `emacs-assistant' directive that this
function used to return for non-local backends turned out to under-
serve small remote models too."
  'emacs-local-assistant)

(defun gptel-agent-runtime-sync-directive-for-current-runtime ()
  "Set `gptel--system-message' according to the active backend/model."
  (let* ((directive (gptel-agent-runtime-directive-for-current-runtime))
         (system-message (alist-get directive gptel-directives)))
    (when system-message
      (setq-local gptel--system-message system-message))
    directive))

(defun gptel-agent-runtime-current-directive-name ()
...
;; Backwards-compat aliases for the legacy `my/gptel-*' names. The host's
;; init may bind these from its own setup or from key bindings; the
;; aliases keep both names working while we move toward MELPA-compliant
;; prefixes. New code should use the `gptel-agent-runtime-*' names
;; directly.
(defvaralias 'my/gptel-local-model-pattern
             'gptel-agent-runtime-local-model-pattern)
(defvaralias 'my/gptel-local-runtime-pattern
             'gptel-agent-runtime-local-runtime-pattern)
(defalias 'my/gptel-directive-for-choice
  #'gptel-agent-runtime-directive-for-choice)
(defalias 'my/gptel-local-runtime-p
  #'gptel-agent-runtime-local-runtime-p)
(defalias 'my/gptel-directive-for-current-runtime
  #'gptel-agent-runtime-directive-for-current-runtime)
(defalias 'my/gptel-sync-directive-for-current-runtime
  #'gptel-agent-runtime-sync-directive-for-current-runtime)
(defalias 'my/gptel-current-directive-name
  #'gptel-agent-runtime-current-directive-name)
...
TOOL OUTPUT FAITHFULNESS (apply WHENEVER you use a tool that returns a list):
- When a tool returns a list of N items (TODOs, files, buffers, search hits,
  trajectories, agents, anything enumerated), your response MUST include all N
  items unless the user EXPLICITLY asked for a summary or a top-K subset.
- Do NOT pick a thematic subset. Do NOT decide some items are more interesting.
  Do NOT add interpretive framing such as 'Based on your recent observations,
  here are your current todos'. The user asked for the list; relay the list.
- If the list is long, render it as a numbered Markdown or Org list with every
  item. Truncation is permitted only when the user said 'top 10', 'first few',
  or similar bounded language.
- If you find yourself shortening a tool result for any reason other than an
  explicit user request, stop and emit every item verbatim.
