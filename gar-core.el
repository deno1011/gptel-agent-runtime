...
seconds and advances the runtime substrate tick so background subscribers
...
(defcustom gptel-agent-runtime-direct-response-destination 'output-buffer
  "Where the autonomous loop renders direct-response output.

Options:
- `output-buffer' (default, safe): always render into a dedicated
  `*gptel-agent-output*' buffer.  User files are never modified by
  the rendering step.  Use this unless you specifically want
  responses inserted into source files.
- `origin-buffer': render into the buffer that was current when
  `gptel-agent-runtime-start' was called -- with file-backed buffers
  still protected (those redirect to the output buffer).  Useful
  when the runtime is being used as an in-place renderer for math
  graphs / explanations in scratch buffers.
- `origin-buffer-unsafe': literally insert into whatever buffer was
  current, including org agenda files and source code.  This was the
  pre-PR-14 behaviour and is preserved only as an escape hatch -- it
  caused real data corruption (run_elisp tool-call transcripts
  appended to user TODO files) before being made non-default.

If you've ever seen `#+begin_tool' or `#+begin_src' blocks appear
in your org TODO files, you were on `origin-buffer-unsafe'."
  :type '(choice (const :tag "Dedicated output buffer (safe)" output-buffer)
                 (const :tag "Origin buffer, file-backed protected" origin-buffer)
                 (const :tag "Origin buffer, unsafe (legacy)" origin-buffer-unsafe))
  :group 'gptel-agent-runtime)

(defcustom gptel-agent-runtime-direct-response-buffer-name
  "*gptel-agent-output*"
  "Buffer name used when `direct-response-destination' is `output-buffer'
or when origin-buffer protection redirects file-backed buffers."
  :type 'string
  :group 'gptel-agent-runtime)

(defcustom gptel-agent-runtime-planner-handover-enabled t
  "When non-nil, the planner prompt prepends a `RECENT SESSIONS' block
listing the last N trajectories in chronological order.

This is the cross-session continuity signal -- complementary to PR
13's similar-trajectories injection.  PR 13 ranks by goal similarity
(`you have done this kind of thing before'); handover ranks by
time (`here is what just happened'), regardless of similarity.

In practice the model uses the two together: similar trajectories
show patterns the new goal echoes; handover shows momentum (what
the user has been working on this week, what just failed)."
  :type 'boolean
  :group 'gptel-agent-runtime)

(defcustom gptel-agent-runtime-planner-handover-count 5
  "Number of most-recent trajectories to inject into the planner prompt
as the `RECENT SESSIONS' block.  0 disables (same as turning off
`planner-handover-enabled')."
  :type 'integer
  :safe #'integerp
  :group 'gptel-agent-runtime)
...
(cl-defstruct (gptel-agent-runtime-trajectory-step
               (:constructor gptel-agent-runtime-trajectory-step-create))
  "Frozen snapshot of one plan-step at session-finalize time.
Part of a `gptel-agent-runtime-trajectory'. Stores enough state to
reconstruct what happened on this step without keeping the live
plan-step struct (which may be garbage-collected once the session is
finalized)."
  step-id
  title
  rationale
  ...
  attempts
  status
  result-status        ; symbol: ok | error | nil
  result-output        ; shortened output string (max 4000 chars)
  result-error
  observations
  reflections)

(cl-defstruct (gptel-agent-runtime-trajectory
               (:constructor gptel-agent-runtime-trajectory-create))
  "Complete observable record of one goal attempt.
Built at session-finalize time by snapshotting the session's plan-steps
plus out-of-band signals filtered to the session's lifetime: verifier
verdicts, matched playbook ids, decisions. This is the substrate the
learning loop (auto-refine playbooks, case-based retrieval) reads from."
  id                   ; uuid; derived from session-id + finalize timestamp
  goal                 ; goal text
  session-id           ; original session id
  started-at           ; ISO timestamp
  finalized-at         ; ISO timestamp
  outcome              ; success | failure | abandoned
  reason               ; finalize REASON symbol (done/failed/max-iterations/...)
  iteration-count
  steps                ; list of trajectory-step, oldest first
  verifier-verdicts    ; list of verdict plists captured during the session window
  playbook-ids         ; list of strings — playbooks that matched at routing
  reflections          ; list of strings (collected from each step + the session)
  decisions            ; copy of session-decisions textual log
  metadata)            ; extension plist
...
Runtime substrate clock value when the evidence was created. AGENT is the
...
(defun gptel-agent-runtime-tools-all ()
...
(defun gptel-agent-runtime-sync-tools ()
  "Enable all registered gptel tools in the current buffer."
  (when (boundp 'gptel--known-tools)
    (setq-local gptel-use-tools t)
    (setq-local gptel-tools (gptel-agent-runtime-tools-all))))


(defun gptel-agent-runtime-select-model ()
...
(global-set-key (kbd "C-c M") #'gptel-agent-runtime-select-model)

(defun gptel-agent-runtime-mode-line ()
  "Show the active gptel model in the mode line."
  (when (bound-and-true-p gptel-mode)
    (format " [%s]" gptel-model)))

(defun gptel-agent-runtime-status ()
...
(defun gptel-agent-runtime-sync-and-status ()
...
(global-set-key (kbd "C-c G s") #'gptel-agent-runtime-status)
(global-set-key (kbd "C-c G S") #'gptel-agent-runtime-sync-and-status)

(unless (member '(:eval (gptel-agent-runtime-mode-line)) mode-line-format)
  (setq mode-line-format
        (append mode-line-format '((:eval (gptel-agent-runtime-mode-line))))))

;; Backwards-compat aliases for the legacy `my/gptel-*' UI command names.
;; Users may have bound these to keys directly or referenced them from
;; setup hooks; the aliases keep both names working until callers move
;; to the `gptel-agent-runtime-*' API.
(defalias 'my/gptel-tools-all      #'gptel-agent-runtime-tools-all)
(defalias 'my/gptel-sync-tools     #'gptel-agent-runtime-sync-tools)
(defalias 'my/gptel-select-model   #'gptel-agent-runtime-select-model)
(defalias 'my/gptel-mode-line      #'gptel-agent-runtime-mode-line)
(defalias 'my/gptel-status         #'gptel-agent-runtime-status)
(defalias 'my/gptel-sync-and-status #'gptel-agent-runtime-sync-and-status)
